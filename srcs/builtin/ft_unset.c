...
/*   Updated: 2025/05/15 14:44:46 by mbrighi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_unset(char *to_remove, char **mini_env)
{
	int i;
	int	j;
	...
	while(mini_env[i] != NULL)
	{
		...
		if ((ft_strncmp(mini_env[i], to_remove, ft_strlen(to_remove)) == 0) &&
		(mini_env[i][ft_strlen(to_remove)] == '='))
		{
			free(mini_env[i]);
			j = i;
			while (mini_env[j] != NULL)
			{
				mini_env[j] = mini_env[j+1];
				j++;
			}
		}
		i++;
